...
- 編集結果を `data/audios/` に保存し、作品/キャラに紐づいた音声履歴へ登録
...
- 生成完了後、または音声編集後に保存された音声: `data/audios/`
...
音声編集画面では、mp3またはwavを読み込み、分割点、カット範囲、音量、ピッチを指定して音声を書き出せます。元ファイルは保存せず、編集後の音声だけを `data/audios/` に保存します。
