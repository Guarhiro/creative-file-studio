...
Node.js v18以上をインストールしてから、アプリのフォルダで依存関係を準備します。

GitHubから取得する場合:

```bash
git clone https://github.com/Guarhiro/creative-file-studio.git
cd creative-file-studio
npm install
npm start
```

Release ZIPを展開して使う場合:
...
現在の版では外部npmパッケージを使っていないため、`npm install` は短時間で完了します。依存関係が追加された版でも同じ手順で起動できます。

## 起動

2回目以降は、アプリのフォルダで以下を実行します。
...
- 簡易ローカル処理では端末内のCanvasで外周につながる余白色の推定/指定と、複数クリックによる領域認識を分けて扱い、許容値と境界ぼかしを調整
- 手動フリーモードではCanvas上でクリック認識、境界指定、ペン除去、復元ペンを使って透過PNGを調整
...
- mp3、wavファイルを読み込み
- 指定した複数の秒数で一度に複数ファイルへ分割
- 不要な開始秒と終了秒を複数指定し、残りの音声をつないで保存
- 0-400%の範囲で音量を変更
- -12から+12半音の範囲でピッチを変更
- 出力形式は元と同じ形式、MP3、WAVから選択
- 音声プレイヤー下に波形と0.1秒単位の現在再生時間を表示
- 編集結果を `data/audios/作品名/キャラクター名/` に保存し、作品/キャラに紐づいた音声履歴へ登録
- 編集後のファイル名横からFinderで保存場所を表示

### 設定とモデル管理

- OpenRouter APIキー、ElevenLabs APIキー、Seedance/Replicate APIキー、ComfyUIクラウドAPIキー、Civitai APIキーをブラウザのlocalStorageに保存
...
- ComfyUI クラウドAPI キー、Forge API キー、Forge Neo API キー、Draw Things API キー、Civitai API キー: ブラウザの localStorage
...
`data/` 配下の作品データ、キャラ設定、プロンプト、取り込み画像、生成物は公開対象から外しています。リポジトリには空フォルダ維持用の `data/.gitkeep` だけを含めます。
...
- Forge URL: Stable Diffusion WebUI Forgeを `--api` 付きで起動し、`http://127.0.0.1:7860` などを指定します。
- Forge APIキー: 必要な環境だけ保存します。
...
音声編集画面では、mp3またはwavを選択またはドラッグ&ドロップで読み込み、分割点、カット範囲、音量、ピッチを指定して音声を書き出せます。元ファイルは保存せず、編集後の音声だけを `data/audios/作品名/キャラクター名/` に保存します。

- 複数分割: 例として10秒、25秒を指定すると、0-10秒、10-25秒、25秒以降の複数ファイルを作成します。
- 不要範囲をカット: 開始秒と終了秒を複数指定し、不要部分を除いた残りを1つの音声として保存します。
- 音量変更: 100%で元と同じ音量、50%で半分、200%で2倍、0%で無音にできます。音量を上げすぎると音割れする場合があります。
- ピッチ変更: 0半音で元の高さ、+12半音で1オクターブ上、-12半音で1オクターブ下に変更します。長さはできるだけ元に近づけて書き出します。
- 出力形式: 元と同じ形式、MP3、WAVから選べます。
- 波形表示: シークバーの下に波形と現在の再生時間を0.1秒単位で表示します。波形をクリックして再生位置を移動できます。
- 再編集: 編集結果または音声編集履歴の「再編集」から、その音声を選択音声として読み込み直せます。
- 作品/キャラ指定: 編集結果を音声履歴へ登録し、キャラ指定ありならキャラの音声一覧にも表示します。
- Finder表示: 編集結果や音声履歴のファイル名横にあるFinderボタンから保存場所を開けます。

音声編集にはffmpeg/ffprobeが必要です。画像編集のbackgroundremoverセットアップ済み環境では、同梱されたffmpeg/ffprobeを利用できます。
